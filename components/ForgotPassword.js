...
const ForgotPassword = () => {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      const data = await axios.post("http://localhost:5000/sendverifyemail", {
        email,
      });

      if (data.data.status === 200 && data.data.message === "Email verified") {
      ...
      if (data.data.status === 404 && data.data.message === "Email not found") {
        setIsLoading(false);
        setDisabled(false);
        toast({
